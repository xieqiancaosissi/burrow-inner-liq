  ...
  const fetchAllTokenHolders = async () => {
    ...
    try {
      ...
      const firstPageResponse = await fetch(
        `https://api.ref.finance/token_holders?number=${numberParam}&page_number=1&page_size=1000`
      );
      ...
      for (let page = 1; page <= totalPages; page++) {
        pagePromises.push(
          fetch(
            `https://api.ref.finance/token_holders?number=${numberParam}&page_number=${page}&page_size=1000`
          ).then((res) => res.json())
        );
      }
  ...
  const formatBalance = (balance: string, tokenId: string) => {
    ...
    if (metadata && metadata.decimals !== undefined) {
      try {
        ...
        console.log(
          `Converted balance: ${balance} -> ${readableNumber} -> ${num}`
        );
